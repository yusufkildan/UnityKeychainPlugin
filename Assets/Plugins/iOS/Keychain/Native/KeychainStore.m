...
NSString * const KeychainStoreErrorDomain = @"com.keychainstore.error_domain";
...
+ (KeychainStore *)keychainStoreWithService:(NSString *)service
{
    return [[self alloc] initWithService:service accessGroup:nil];
}

+ (KeychainStore *)keychainStoreWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
...
{
    return [[self alloc] initWithServer:server protocolType:protocolType authenticationType:KeychainStoreAuthenticationTypeDefault];
}

+ (KeychainStore *)keychainStoreWithServer:(NSURL *)server protocolType:(KeychainStoreProtocolType)protocolType authenticationType:(KeychainStoreAuthenticationType)authenticationType
{
...
- (instancetype)initWithService:(NSString *)service accessGroup:(NSString *)accessGroup
{
    self = [super init];
    if (self) {
        _itemClass = KeychainStoreItemClassGenericPassword;
...
{
    return [self initWithServer:server protocolType:protocolType authenticationType:KeychainStoreAuthenticationTypeDefault];
}

- (instancetype)initWithServer:(NSURL *)server protocolType:(KeychainStoreProtocolType)protocolType authenticationType:(KeychainStoreAuthenticationType)authenticationType
{
    self = [super init];
    if (self) {
        _itemClass = KeychainStoreItemClassInternetPassword;
...
- (void)commonInit
{
    _accessibility = KeychainStoreAccessibilityAfterFirstUnlock;
}
...
+ (NSString *)stringForKey:(NSString *)key service:(NSString *)service accessGroup:(NSString *)accessGroup error:(NSError *__autoreleasing *)error
{
    ...
    KeychainStore *keychain = [KeychainStore keychainStoreWithService:service accessGroup:accessGroup];
...
+ (NSArray UIC_KEY_TYPE *)allKeysWithItemClass:(KeychainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == KeychainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
    } else if (itemClass == KeychainStoreItemClassInternetPassword) {
...
+ (NSArray *)allItemsWithItemClass:(KeychainStoreItemClass)itemClass
{
    CFTypeRef itemClassObject = kSecClassGenericPassword;
    if (itemClass == KeychainStoreItemClassGenericPassword) {
        itemClassObject = kSecClassGenericPassword;
...
- (void)setAccessibility:(KeychainStoreAccessibility)accessibility authenticationPolicy:(KeychainStoreAuthenticationPolicy)authenticationPolicy
{
...
- (NSMutableDictionary *)attributesWithKey:(NSString *)key value:(NSData *)value error:(NSError *__autoreleasing *)error
{
    ...
    if (_authenticationPolicy && accessibilityObject) {
        ...
        if (floor(NSFoundationVersionNumber) <= floor(iOS_7_1_or_10_9_2) && _accessibility == KeychainStoreAccessibilityWhenPasscodeSetThisDeviceOnly) {
#if TARGET_OS_IOS
            NSLog(@"%@", @"Unavailable 'KeychainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on iOS versions prior to 8.0.");
#else
            NSLog(@"%@", @"Unavailable 'KeychainStoreAccessibilityWhenPasscodeSetThisDeviceOnly' attribute on OS X versions prior to 10.10.");
#endif
        } else {
...
{
    switch (_itemClass) {
        case KeychainStoreItemClassGenericPassword:
            return kSecClassGenericPassword;
        case KeychainStoreItemClassInternetPassword:
...
- (CFTypeRef)protocolTypeObject
{
    switch (_protocolType) {
        case KeychainStoreProtocolTypeFTP:
            return kSecAttrProtocolFTP;
        case KeychainStoreProtocolTypeFTPAccount:
            return kSecAttrProtocolFTPAccount;
        case KeychainStoreProtocolTypeHTTP:
            return kSecAttrProtocolHTTP;
        case KeychainStoreProtocolTypeIRC:
            return kSecAttrProtocolIRC;
        case KeychainStoreProtocolTypeNNTP:
            return kSecAttrProtocolNNTP;
        case KeychainStoreProtocolTypePOP3:
            return kSecAttrProtocolPOP3;
        case KeychainStoreProtocolTypeSMTP:
            return kSecAttrProtocolSMTP;
        case KeychainStoreProtocolTypeSOCKS:
            return kSecAttrProtocolSOCKS;
        case KeychainStoreProtocolTypeIMAP:
            return kSecAttrProtocolIMAP;
        case KeychainStoreProtocolTypeLDAP:
            return kSecAttrProtocolLDAP;
        case KeychainStoreProtocolTypeAppleTalk:
            return kSecAttrProtocolAppleTalk;
        case KeychainStoreProtocolTypeAFP:
            return kSecAttrProtocolAFP;
        case KeychainStoreProtocolTypeTelnet:
            return kSecAttrProtocolTelnet;
        case KeychainStoreProtocolTypeSSH:
            return kSecAttrProtocolSSH;
        case KeychainStoreProtocolTypeFTPS:
            return kSecAttrProtocolFTPS;
        case KeychainStoreProtocolTypeHTTPS:
            return kSecAttrProtocolHTTPS;
        case KeychainStoreProtocolTypeHTTPProxy:
            return kSecAttrProtocolHTTPProxy;
        case KeychainStoreProtocolTypeHTTPSProxy:
            return kSecAttrProtocolHTTPSProxy;
        case KeychainStoreProtocolTypeFTPProxy:
            return kSecAttrProtocolFTPProxy;
        case KeychainStoreProtocolTypeSMB:
            return kSecAttrProtocolSMB;
        case KeychainStoreProtocolTypeRTSP:
            return kSecAttrProtocolRTSP;
        case KeychainStoreProtocolTypeRTSPProxy:
            return kSecAttrProtocolRTSPProxy;
        case KeychainStoreProtocolTypeDAAP:
            return kSecAttrProtocolDAAP;
        case KeychainStoreProtocolTypeEPPC:
            return kSecAttrProtocolEPPC;
        case KeychainStoreProtocolTypeNNTPS:
            return kSecAttrProtocolNNTPS;
        case KeychainStoreProtocolTypeLDAPS:
            return kSecAttrProtocolLDAPS;
        case KeychainStoreProtocolTypeTelnetS:
            return kSecAttrProtocolTelnetS;
        case KeychainStoreProtocolTypeIRCS:
            return kSecAttrProtocolIRCS;
        case KeychainStoreProtocolTypePOP3S:
...
- (CFTypeRef)authenticationTypeObject
{
    switch (_authenticationType) {
        case KeychainStoreAuthenticationTypeNTLM:
            return kSecAttrAuthenticationTypeNTLM;
        case KeychainStoreAuthenticationTypeMSN:
            return kSecAttrAuthenticationTypeMSN;
        case KeychainStoreAuthenticationTypeDPA:
            return kSecAttrAuthenticationTypeDPA;
        case KeychainStoreAuthenticationTypeRPA:
            return kSecAttrAuthenticationTypeRPA;
        case KeychainStoreAuthenticationTypeHTTPBasic:
            return kSecAttrAuthenticationTypeHTTPBasic;
        case KeychainStoreAuthenticationTypeHTTPDigest:
            return kSecAttrAuthenticationTypeHTTPDigest;
        case KeychainStoreAuthenticationTypeHTMLForm:
            return kSecAttrAuthenticationTypeHTMLForm;
        case KeychainStoreAuthenticationTypeDefault:
...
- (CFTypeRef)accessibilityObject
{
    switch (_accessibility) {
        case KeychainStoreAccessibilityWhenUnlocked:
            return kSecAttrAccessibleWhenUnlocked;
        case KeychainStoreAccessibilityAfterFirstUnlock:
            return kSecAttrAccessibleAfterFirstUnlock;
        case KeychainStoreAccessibilityAlways:
            return kSecAttrAccessibleAlways;
        case KeychainStoreAccessibilityWhenPasscodeSetThisDeviceOnly:
            return kSecAttrAccessibleWhenPasscodeSetThisDeviceOnly;
        case KeychainStoreAccessibilityWhenUnlockedThisDeviceOnly:
            return kSecAttrAccessibleWhenUnlockedThisDeviceOnly;
        case KeychainStoreAccessibilityAfterFirstUnlockThisDeviceOnly:
            return kSecAttrAccessibleAfterFirstUnlockThisDeviceOnly;
        case KeychainStoreAccessibilityAlwaysThisDeviceOnly:
...
{
    NSError *error = [NSError errorWithDomain:KeychainStoreErrorDomain code:KeychainStoreErrorInvalidArguments userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:KeychainStoreErrorDomain code:-67594 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
...
#endif
    NSError *error = [NSError errorWithDomain:KeychainStoreErrorDomain code:status userInfo:@{NSLocalizedDescriptionKey: message}];
...
{
    NSError *error = [NSError errorWithDomain:KeychainStoreErrorDomain code:-99999 userInfo:@{NSLocalizedDescriptionKey: message}];
    NSLog(@"error: [%@] %@", @(error.code), error.localizedDescription);
    return error;
}
